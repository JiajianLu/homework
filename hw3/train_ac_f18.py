    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(len(re_n)):
            if terminal_n[i] == 1:
                ...
                V_s_p = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no : [next_ob_no[i]]})
            Q_n = re_n[i] + self.gamma * V_s_p
            V_s = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no : [ob_no[i]]})
            adv_n.append(Q_n - V_s)
        ...
        for _ in range(self.num_target_updates):
            ...
            for i in range(len(re_n)):
                if terminal_n[i] == 1:
                    ...
                    V_s_p = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no : [next_ob_no[i]]})
                Q.append(re_n[i] + self.gamma * V_s_p)
