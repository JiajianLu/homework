...
class QLearner(object):
    ...
    """
        ...
        y = (self.rew_t_ph + gamma * tf.reduce_max(self.q_target, axis=1))

        if double_q:
            y = (self.rew_t_ph + gamma *
                 tf.reduce_sum(self.q_target * tf.one_hot(tf.argmax(self.q_current, axis=1), self.num_actions), axis=1))

        loss_fn = tf.reduce_sum(self.q_current * tf.one_hot(self.act_t_ph, self.num_actions), axis=1) - \
                  (1.0 - self.done_mask_ph) * y

        self.total_error = tf.reduce_mean(huber_loss(loss_fn))
    ...
    def step_env(self):
        ...
        """
    What must go in the buffer? 
    Current State, Current Action, Reward, Future State


    store the last state in the buffer

    Get the input for the NN from the buffer (it must be preprocessed, or attach some older frames)

    find the action to take (greedy)

    take the action, get the new state , reward and done


    store the action, reward, and done for that state

    if done, then reset the state
    ...
    """
        ...
        if not self.model_initialized or random.random() < self.exploration.value(self.t):
            ...
            action = self.session.run(self.q_current, feed_dict={self.obs_t_ph: [obs]})
            action = np.argmax(action)

        obs_next, reward_current, done, _ = self.env.step(action)
        ...
        self.replay_buffer.store_effect(idx, action, reward_current, done)
    ...
    def log_progress(self):
        ...
        if self.t % self.log_every_n_steps == 0 and self.model_initialized:
            with open('result_lunar_lander_single_q_lr_1e-2_1_million.csv', 'a+') as file:
                file.write  (str(self.t)+","+str(self.mean_episode_reward)+","+str(self.best_mean_episode_reward)+","
                           +str(len(episode_rewards))+","+str(self.exploration.value(self.t))+"," +
                           str(self.optimizer_spec.lr_schedule.value(self.t))+"\n")
