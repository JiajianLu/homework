...
#                           ----------PROBLEM 2----------
#========================================================================================#  
def build_mlp(input_placeholder, output_size, scope, n_layers=2, size=32, activation=tf.tanh, output_activation=None):
	"""
...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        # YOUR CODE HERE Dimension for the davantage
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, 'NN_with_discrete', self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, 'NN_with_continue', self.n_layers, self.size)
            sy_logstd = tf.Variable(tf.zeros([1,self.ac_dim]), name="log_std", dtype=tf.float32) #variable to train
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            b_n = ((b_n - np.mean(b_n)) / (np.std(b_n)+1e-8)) * np.std(q_n) + np.mean(q_n)
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR_CODE_HERE
            target_n = q_n
            target_n = (target_n - np.mean(target_n))/(np.std(target_n)+1e-8)
        ...
        # YOUR_CODE_HERE
        self.sess.run(self.update_op, feed_dict={self.sy_adv_n : adv_n, self.sy_ob_no : ob_no, self.sy_ac_na : ac_na})
